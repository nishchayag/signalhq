...
- **Multi-tenant migration is complete** — every route that creates a Question or Message stamps `organizationId`:
  ...
  - `app/api/sendMessage/route.ts` (the original `/u/[username]` flow) stamps `organizationId` via `lib/orgContext.ts#getPersonalOrganizationId` (the recipient's oldest Membership — session-independent, since this route only ever has a `userId`, not a request session). `app/api/acceptMessages/route.ts` remains legacy/userId-only, but doesn't need to touch `organizationId` — it only flips the `isAcceptingMessages` flag on `User`, which isn't an org-scoped record.
