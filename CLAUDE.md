...
- **Migration status is mixed, not complete** — check before assuming a route is org-scoped:
  - Already cut over: `app/api/questions/*` and `app/api/getMessages/route.ts` use `resolveActiveContext` + filter by `organizationId` (with team-scoped visibility for non-admin members via a `teamScopeFilter` helper — OWNER/ADMIN see everything, MEMBERs see org-level + their own teams' questions).
  - `app/api/sendMessage/route.ts` (the original `/u/[username]` flow) now stamps `organizationId` too, via `lib/orgContext.ts#getPersonalOrganizationId` (the recipient's oldest Membership — session-independent, since this route only ever has a `userId`, not a request session). `app/api/acceptMessages/route.ts` remains legacy/userId-only, but doesn't need to touch `organizationId` — it only flips the `isAcceptingMessages` flag on `User`, which isn't an org-scoped record.
  - `app/api/admin/migrate/route.ts` is a **temporary, idempotent one-time backfill** (gated by `x-migration-secret` header matching `MIGRATION_SECRET` env var, dry-run by default unless `?dryRun=false`) that gives every existing user a personal Organization + OWNER Membership and stamps `organizationId` onto their pre-existing Questions/Messages. Remove once the live backfill is verified — don't extend it.
...
- JWT session strategy. Custom fields (`_id`, `username`, `isVerified`, `isAcceptingMessages`, `name`, `activeOrgId`, `activeOrgSlug`, `activeOrgRole`) are threaded through the `jwt` and `session` callbacks and typed in `types/next-auth.d.ts` — update all three when adding a session field.
...
- Route handlers return `NextResponse.json(...)`. Many existing handlers return `{ success, error/message }` with a **200 status even on logical failure** (e.g. `sendMessage`, `getMessages`) — newer handlers (`questions/`, `organizations/`) use proper status codes (401/400/403/500). Prefer the status-code style for new code, but check what the calling client expects.
...
- Public feedback pages: `app/u/[username]/` (legacy personal link), `app/q/[slug]/` (question-specific, legacy), and the org-scoped equivalents `app/o/[orgSlug]/` and `app/o/[orgSlug]/q/[slug]/`. Authed area: `app/dashboard/` (`old-page.tsx` is a superseded version) plus `app/dashboard/organization/` for org/team/member management.
...
Required at runtime (no `.env.example` committed): `MONGODB_URI`, `NEXTAUTH_SECRET`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `NEXT_PUBLIC_BASE_URL`, and optional analytics IDs `NEXT_PUBLIC_GA_ID`, `NEXT_PUBLIC_CLARITY_ID`. `MIGRATION_SECRET` gates the one-time backfill route (`app/api/admin/migrate`). External image hosts must be whitelisted in `next.config.ts` `images.domains`.
