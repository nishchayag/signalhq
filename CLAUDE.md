...
- **Migration status is mixed, not complete** — check before assuming a route is org-scoped:
  ...
  - **Not yet migrated** (legacy, userId/createdFor-only): `app/api/sendMessage/route.ts` (the original `/u/[username]` flow) and `app/api/acceptMessages/route.ts` — these don't touch `organizationId` at all, so messages sent through the legacy personal link never get stamped with an org.
